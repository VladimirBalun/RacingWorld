...
Graphics::Components::SimpleMesh::SimpleMesh()
{
    mesh = new Graphics::Tools::HardwareBuffers();
    shader = new Graphics::Tools::ShaderProgram("Resources\\Shaders\\BaseShader.vert", "Resources\\Shaders\\BaseShader.frag");
    positionLocation = shader->getAttribLocation("position");
}

void Graphics::Components::SimpleMesh::createCube(float size)
{
    for (float &n : cube_v)
        n *= size;
...
void Graphics::Components::SimpleMesh::createTetrahedron(float size)
{
...
void Graphics::Components::SimpleMesh::drawMesh(TypeRender tr)
{
